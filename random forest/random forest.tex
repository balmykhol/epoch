...
\title{
%	\logo{
Random Forest\\Epoch IIT Hyderabad
%	}
}					
...
\indent The generalization error of a forest of tree classifiers depends on the strength of the individual trees in the forest and the correlation between them. Using a random selection of features to split each node yields error rates that compare favourably to Adaboost.\\
\indent It is a Supervised Machine Learning Algorithm that is used widely in Classification and Regression problems. The algorithm's strength lies in its ability to handle complex datasets and mitigate over fitting, making it a valuable tool for various predictive tasks in machine learning.
\begin{center}
...
\begin{center}
\includegraphics[scale=0.35]{randomforest.png}
\end{center}
\newpage
\begin{center}
...
\begin{enumerate}
\item Diversity
\item Immune to curse of dimensionality
\item Parallelization
\item No need for Train-Test split 
\item Stability
\end{enumerate}
\begin{center}
...
\end{center}
\begin{itemize}
\item n\_estimators: Number of trees the algorithm builds before averaging the predictions.
\item max\_features: Max features the forest considers splitting a node.
\item mini\_sample\_leaf: Minimum number of leaves required to split a internal node.
\item criterion: The criteria to split the node.(entropy/gini/log loss)
\item max\_leaf\_nodes: Max leaf nodes in a tree.
\item n\_jobs: It tells the engine how many processors are allowed to be used. If the value is -1, it uses the max possible.
\end{itemize}
\begin{center}
\textbf{Conclusion}
\end{center}
